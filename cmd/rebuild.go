...
func (cmd *rebuildCmd) shouldHaveMagicComment(dst string) error {
	r, err := os.Open(dst)
	if err != nil {
		return err
	}
	...
	// NOTE: magicComment[0] is the longest line in magicComment
	br := bufio.NewReaderSize(r, len(magicComment[0])+1)
	buf := make([]byte, 0, len(magicComment[0])+1)
	for i := range magicComment {
		n, err := cmd.readLine(br, buf)
		if i == 1 { // original file path
			n = len(magicComment[i])
		}
		if err != nil || string(buf[:n]) != magicComment[i] {
...
func (*rebuildCmd) readLine(br *bufio.Reader, buf []byte) (int, error) {
	buf = buf[:0]
	var n int
	for {
		b, more, err := br.ReadLine()
		n += len(b)
		buf = append(buf, b...)
		if err != nil {
			return 0, err
...
func (*rebuildCmd) copyFileWithMagicComment(src, dst string) error {
	...
	for i := range magicComment {
		_, err = bw.WriteString(magicComment[i])
		if err != nil {
			return err
		}
		if i == 1 { // original file path
			_, err = bw.WriteString(src)
			if err != nil {
				return err
			}
		}
